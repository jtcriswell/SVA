...
sva_ipush_function5 (void (*newf)(uintptr_t, uintptr_t, uintptr_t),
  ...
  *(--(ep->rsp)) = 0x0fec;
  ...
  /*
   * Instead of assigning 1 to ep->valid for marking the interrupt
...
#endif
    /*
     * Instead of assigning 0 to icontextp->valid for marking the interrupt
     * context as invalid, we turn the LSB of valid off (clear the LSB)
     */
